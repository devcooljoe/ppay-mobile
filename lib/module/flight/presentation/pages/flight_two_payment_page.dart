...
@RoutePage()
class FlightTwoPaymentPage extends HookConsumerWidget {
  const FlightTwoPaymentPage({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final from = confirmation?.outbound.isNotEmpty == true ? confirmation!.outbound.first.airportFrom : '';
    final to = confirmation?.outbound.isNotEmpty == true ? confirmation!.outbound.first.airportTo : '';
    final duration = confirmation?.outbound.isNotEmpty == true ? FlightFormatUtils.formatDuration(confirmation!.outbound.first.duration) : '';
    final walletBalance = walletAsync.value?.balance ?? '0.00';

    Future<void> _pay() async {
      final passengers = ref.read(passengerFormDataProvider);
      if (passengers.isEmpty) { MessageHandler.showErrorSnackBar(context, 'No passenger data found'); return; }
      if (confirmation == null) { MessageHandler.showErrorSnackBar(context, 'No flight confirmation found'); return; }

      final verified = await showSecurityPinBottomsheet(context);
      if (!verified) return;

      await ref.read(bookFlightProvider.notifier).call(offerId: confirmation.id, passengers: passengers);
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text('Payment', style: TextStyle(fontFamily: 'InstrumentSans', color: PPaymobileColors.mainScreenBackground, fontSize: 18.sp, fontWeight: FontWeight.w500)),
        leading: Padding(
          ...
          Expanded(
            child: ListView(
              children: [
                    ...
                    Row(
                      ...
                      children: [
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(from, style: TextStyle(fontFamily: 'InstrumentSans', color: PPaymobileColors.mainScreenBackground, fontSize: 24.sp, fontWeight: FontWeight.w600)),
                          4.verticalSpace,
                        ...
                        SizedBox(
                          ...
                          child: Stack(alignment: Alignment.center, children: [
                            ...
                            Positioned(bottom: 0.h, child: Text(duration, style: TextStyle(fontFamily: 'InstrumentSans', color: PPaymobileColors.mainScreenBackground, fontSize: 14.sp, fontWeight: FontWeight.w500))),
                          ]),
                        ),
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(to, style: TextStyle(fontFamily: 'InstrumentSans', color: PPaymobileColors.mainScreenBackground, fontSize: 24.sp, fontWeight: FontWeight.w600)),
                          4.verticalSpace,
                ...
                24.verticalSpace,
                Padding(padding: EdgeInsets.symmetric(horizontal: 20.w), child: Text('Payment From', style: TextStyle(fontFamily: 'InstrumentSans', color: Colors.black, fontSize: 16.sp, fontWeight: FontWeight.w600))),
                10.verticalSpace,
                Padding(
                  padding: EdgeInsets.symmetric(horizontal: 20.w),
                  child: Container(
                    height: 74.h, width: double.infinity,
                    padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 9.h),
                    decoration: BoxDecoration(border: Border.all(color: PPaymobileColors.filterBorderColor, width: 1.w), borderRadius: BorderRadius.circular(9).r),
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      Row(children: [
                        Container(height: 39.h, width: 37.w, padding: EdgeInsets.symmetric(horizontal: 7.w, vertical: 8.h), decoration: BoxDecoration(borderRadius: BorderRadius.circular(4).r, color: const Color(0xffEFF5F2)), child: Center(child: SizedBox(height: 23.w, width: 23.w, child: Image.asset('assets/images/flag.png', fit: BoxFit.contain)))),
                        9.horizontalSpace,
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                65.verticalSpace,
                Padding(padding: EdgeInsets.symmetric(horizontal: 20.w), child: Text('Order Summary', style: TextStyle(fontFamily: 'InstrumentSans', color: Colors.black, fontSize: 18.sp, fontWeight: FontWeight.w600))),
                18.verticalSpace,
                Padding(
                  padding: EdgeInsets.symmetric(horizontal: 20.w),
                  child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
